...
let rec subst arg s = function
      ...
      | Call (f, OpList ops) -> Call(f, List.map (subst arg s) ops |> OpList)
      | Call (f, Unfold(op,op2))  -> Call(f, Unfold(subst arg s op, subst arg s op2))
...
let rec allIds = function
    ...
    | Call (_, OpList ops) -> List.fold(fun s op -> op |> allIds |> Set.union s) Set.empty<_> ops 
    | Call (_, Unfold(op,op2))  -> Set.union (allIds op) (allIds op2)
    | Value _ -> Set.empty<_>
...
let freeIds x =
    let rec halp bound = function
        ...
        | Call (f, OpList ops)  -> List.fold(halp) bound ops 
        | Call (f, Unfold(op,op2))  -> Set.union (halp bound op) (halp bound op2)
        | Value _ -> bound
...
let rename all (t, s, x) =
    ...
    let rec halp = function
        ...
        | Let(sc,v,op) -> match halp op with 
                          ...
                          | _ -> Fine
        | Value _ -> Fine
        | Call (f, Unfold(op,op2)) -> 
            match halp op with
            | Renamed rop -> Renamed (Call (f, Unfold(rop,op2)))
            | _ ->
                match halp op2 with
                | Renamed rop2 -> Renamed (Call (f, Unfold(op,rop2)))
                | _ -> Fine
        | Call (f, OpList ops) -> 
            ops 
            ...
            |> List.fold (fun state (op,rop) -> 
                          ...
                          | Renamed(Call(f,OpList ops)),_ -> Renamed(Call(f,OpList (op::ops)))
                          | Renamed (_) as r,_ -> r
                          | Fine,Renamed rop -> Renamed(Call(f,OpList [rop]))
                          | Fine,Fine -> Fine) Fine 
            |> function 
            | Renamed(Call(f,OpList ops)) -> Renamed(Call(f,OpList <| List.rev ops))
            | Renamed (_) as r -> r   
            | _ -> Fine
...
let rec unfold f op op2 env = 
      ...
      let newTimes = 
            times |> Distribution.map(fun times' ->
                        ...
                        reduce env (Call(f, OpList newOps))
            ) 
...
and reduce (env:Environment) (operation:Operation) = 
      ...
      | Call(func,ops) -> call func ops env

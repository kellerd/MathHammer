...
let rootLocation model =
    ...
    g 
        [ SVGAttr.Transform 
          <| sprintf "translate(%d,%d)" model.Dimensions.Left 
                 model.Dimensions.Top ] 
        [ svg [ SVGAttr.Width model.Dimensions.Width
                SVGAttr.Height model.Dimensions.Height ] [ rect 
                                                               [ SVGAttr.Width 
...
let root model dispatch =
    let models =
        ...
        |> List.map 
               (fun (_, m) -> 
               MathHammer.Models.View.root m 
                   (fun msg -> ModelMsg(msg, m.Name) |> dispatch))

...
let nestOps ops returning =
    ops
    |> List.rev
    |> List.reduce (>>)
    <| returning

let createArgs argList operationBody =
    argList
    |> List.rev
    |> List.fold (fun f arg -> lam arg f) operationBody

let applyArgs body argValues = body :: argValues |> List.reduce apply
...
let freeIds x =
    let rec halp bound =
        function 
        | Var(v) -> 
            if Set.contains v bound then Set.empty
...
let idNum (s : string) =
    let rec halp i =
        if i = -1 || not (isDigit s.[i]) then i
        else halp (i - 1)
    
    let stop = s.Length - 1
    let res = halp stop
    if res = stop then s, 1
...
let uniqueId taken s =
    ...
    let rec halp i =
        ...
        if Set.contains newId taken then halp (i + 1)
        else newId
    halp (start + 1)
...
let tryFindLabel name operation =
    let (|FirstIsName|_|) =
        function 
        | Value(ParamArray([ Value(Str name'); v ])) when name' = name -> Some v
        | _ -> None
...
let closure env op =
    let rec fixGp env =
        function 
        | NoValue | Str _ | Float _ | Int _ as gp -> gp
...
let nestCheck check op =
    let rec fixGp check =
        function 
        | NoValue | Str _ | Float _ | Int _ as gp -> gp |> check
...
let rec evalCall func v env : Operation =
    let repeat lam op2 =
        ...
        let rec repeatOps times : Operation =
            let rec repeatOp : GamePrimitive -> Operation =
                function 
                | NoValue -> create 0
                | Check(Check.Fail(m)) -> 
    ...
    | Count, Value(ParamArray []) -> 
        (GamePrimitive.Zero, GamePrimitive.Zero)
        |> tuple
        |> Value
...
and evalOp env (operation : Operation) : Operation =
    let (|MadeChoice|_|) env op : Operation option =
        ...
        | Choice(key, choices) -> 
            ...
            |> Option.bind (function 
                   | (Value(Str name)) -> List.tryFind (fst >> (=) name) choices
                   | _ -> None)
    ...
    let r =
        ...
        | App(Lam(x, op), value) -> evalOp (Map.add x value env) op
        | App(f, value) -> 

...
let squareParen react =
    [ str "["
      react
      str "]" ]
    |> ofList

let rec unparseCheck unparseV =
    function 
    | Check.Pass(v) -> 
        span [ Style [ Color(colour 255.) ] ] [ str "Pass: "
                                                unparseV v ]
    | Check.Fail(v) -> 
        span [ Style [ Color(colour 0.) ] ] [ str "Fail: "
                                              unparseV v ]
    ...
    | _ -> 
        ...
        |> List.map (fun (r, prob) -> 
               let colourValue =
                   ...
                   | Tuple(Int x, Int y) -> 
                       (float x / (float x + float y)) * float 0xFF
                       |> colourA
                       |> Some
...
and unparse unparseValue operation =
    ...
    | App(Call Count, x) -> 
        [ str "(Passes,Fails) in "
          unparse unparseValue x ]
        |> ofList
    | App(Lam(_, x), _) -> unparse unparseValue x //paren (unparse (Lam(p,x))) + " " + argstring a
    | App(f, (Var(v))) -> 
        [ unparse unparseValue f
          str v ]
        |> ofList
    | App(f, a) -> 
        [ unparse unparseValue f
          unparse unparseValue a ]
        |> ofList
    | Let(_, _, inner) -> unparse unparseValue inner
    | PropertyGet(s, op) -> 
        [ unparse unparseValue op
          str ("." + s) ]
        |> ofList
    | IfThenElse(ifExpr, thenExpr, elseExpr) -> 
        let ifPart =
            [ str "if "
              unparse unparseValue ifExpr ]
            |> ofList
        
        let thenPart =
            [ str " then "
              br []
              unparse unparseValue thenExpr ]
            |> ofList
        
        let elsePart =
            Option.map (fun elseExpr -> 
                [ br []
                  unparse unparseValue elseExpr ]) elseExpr
            |> Option.toList
            |> List.collect id

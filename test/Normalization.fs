...
let ``Lambda Calculus`` =
    ...
    testList "Lambda Calculus Normalization" (List.map doTest [ zero, 0
                                                                one, 1
                                                                two, 2
                                                                three, 3 ])
...
let ``Ski Combinators`` =
    ...
    let ``or`` = k
    let ``and`` = ``false``
    testList "Ski Combinators" [ test "I is identity" { 
                                     let x' = vInt 6
                                     App(i, x')
                                     |> normalize
                                     |> snd
                                 ...
                                 testList "Test Boolean Functions" [ test "F returns second" { 
                                                                         let x' = vInt 6
                                                                         let y' = vInt 8
                                                                         App(App(``false``, x'), y')
                                                                         |> normalize
...
let ``Closure test`` =
    test "Inside lam scopes correctly" { 
        App(Lam("y", App(Lam("y", Var "y"), Value(Int(7)))), Value(Int(9)))
        |> normalize
        |> snd
        ==? Value(Int(7))
    }
...
let ``Getting a property test`` =
    ...
    let c =
        opList [ opList [ Value(Str "M")
                          Value(Int 8) ]
                 opList [ Value(Str "T")
                          six ] ]
    testList "Getting a property test" [ test "Get First Property" { 
                                             PropertyGet("M", c)
                                             |> normalize
                                             |> snd
                                             ==? Value(Int 8)
...
let Choices =
    ...
    let ops = [ op1; op4; op8; op11; op15; op16; op8; op11; op15; op16 ]
    let ops2 = [ op2; op5; op9; op12; op17; op2; op5; op9; op12; op17 ]
    let opsBoth = [ op3; op6; op7; op10; op13; op14; op18; op19; op20; op21 ]
    
    let createTests f ops title expected =
        ops
        |> List.mapi (fun i op -> 
               test (sprintf "%s %d" title i) { 
                   f op
                   |> normalize
                   |> fst
                   ==? expected
    ...
    [ createTests id ops "Value Fst" expected
      ...
      createTests (fun (op, op2) -> 
          Choice("Test", 
                 [ "Op", op
                   "Op2", op2 ])) (List.zip ops ops) "Choice Fst" (Map.mergeSets expected choiceTest)
      createTests (fun (op, op2) -> 
          Choice("Test", 
                 [ "Op", op
                   "Op2", op2 ])) (List.zip ops2 ops2) "Choice Snd" (Map.mergeSets expected2 choiceTest)
      createTests (fun (op, op2) -> 
          Choice("Test", 
                 [ "Op", op
                   "Op2", op2 ])) (List.zip opsBoth opsBoth) "Choice Both1" (Map.mergeSets expectedBoth choiceTest)
      createTests (fun (op, op2) -> 
          Choice("Test", 
                 [ "Op", op
                   "Op2", op2 ])) (List.zip ops ops2) "Choice Fst2" (Map.mergeSets expectedBoth choiceTest)
      createTests (fun (op, op2) -> 
          Choice("Test", 
                 [ "Op", op
                   "Op2", op2 ])) (List.zip ops2 ops) "Choice Snd2" (Map.mergeSets expectedBoth choiceTest)
      createTests (fun (op, op2) -> 
          Choice("Test", 
                 [ "Op", op
                   "Op2", op2 ])) (List.zip ops2 opsBoth) "Choice Both2" (Map.mergeSets expectedBoth choiceTest)
      createTests (fun (op, op2) -> 
          Choice("Test", 
                 [ "Op", op
                   "Op2", op2 ])) (List.zip opsBoth ops) "Choice Both3" (Map.mergeSets expectedBoth choiceTest) ]
    |> testList "Choices"

let ``Counting call test`` =
    let appliedTwo = two <*> vInt 7 <*> vInt 7
    testList "Normalize function application" [ test "Count by Param Array" { 
                                                    let count ops = App(Call Count, opList (ops))
                                                    
                                                    let normalizedFirst =
                                                        count [ normalize appliedTwo |> snd
                                                                normalize appliedTwo |> snd
                                                                normalize appliedTwo |> snd ]
                                                        |> normalize
                                                        |> snd
                                                    
                                                    let normalizedSecond =
                                                        count [ appliedTwo; appliedTwo; appliedTwo ]
                                                        |> normalize
                                                        |> snd
...
let tests =
    testList "Normalization Tests" 
        [ ``Counting call test``; ``Getting a property test``; ``Lambda Calculus``; ``Ski Combinators``; ``Closure test``; Choices ]

...
let ``Operation Tests`` = 
    testList "Operation Tests" [
        ...
        let retValueIsSame v f =
            ...
            evaled ==? evaled'
        yield test "Return value is same evaluated without normalization" { retValueIsSame v id}
        yield test "Return value is same evaluated with normalization" {retValueIsSame v normalizeOp}

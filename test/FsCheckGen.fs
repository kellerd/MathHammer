...
type ListDistScalarType =
    | ScalarGamePrimitive of GamePrimitive
    | DistGamePrimitive of GamePrimitive
    | ListGamePrimitive of GamePrimitive
    member x.ToGamePrimitive() = x |> function 
                                 | ScalarGamePrimitive g | DistGamePrimitive g | ListGamePrimitive g -> g
...
let genCheck ofType =
    Gen.oneof [ Gen.map (Check.Pass) ofType
...
let genGpDist gen =
    ...
    let pair = gen |> Gen.map (fun ls -> List.zip (ls) <| (Gen.sample (List.length ls) probabilities |> List.ofArray) |> Distribution.create)
    Gen.map Dist pair

let genScalarType =
    Gen.oneof [ Gen.map Check (genCheck genPrimitive)
                (genNumber id) ]
    |> Gen.map ScalarType
...
let genTwoSimilarTypes =
    let positiveInt =
        genInt |> Gen.filter (fun v -> 
                      match v with
                      | Int i when i >= 0 -> true
                      | _ -> false)
    
    let positiveFloat =
        genFloat |> Gen.filter (fun v -> 
                        match v with
...
let genTwoSimilarScalarTypes =
    ...
    Seq.collect similarCombo [ positiveInt; positiveFloat ]
    |> Gen.oneof
    |> Gen.map TwoSimilarScalarTypes

let genDistType = genGpDist genListOfPrimitive |> Gen.map DistType

let genListType =
    Gen.map (List.map Value
             >> ParamArray
             >> ListType) genListOfPrimitive
...
let genListDistScalarType =
    ...
    |> Gen.oneof
    |> Gen.map (function 
           | Dist(d) -> DistGamePrimitive(Dist(d))
           | ParamArray(ops) -> ListGamePrimitive(ParamArray(ops))
           | Int _ | Str _ | Float _ | Check _ | NoValue _ | Tuple _ as g -> ScalarGamePrimitive g)
...
let genOp =
    let rec genOp' =
        ...
        | n when n > 0 -> 
            ...
            let lambda = Gen.map2 (fun (NonEmptyString n) body -> Lam(n, body)) Arb.generate<_> subtree
            Gen.oneof [ Gen.map Value Arb.generate<GamePrimitive>
                        ...
                        Gen.oneof [ Gen.map2 (fun lam value -> App(lam, value)) lambda subtree
                                    Gen.map (fun d -> App(Call Dice, Value(Int d))) Arb.generate<_>
                                    Gen.map (fun ops -> 
                                        App(Call(Product), 
                                            (ops
                                             |> List.map Value
                                             |> opList))) (genNumber (Gen.listOf))
                                    Gen.map (fun ops -> 
                                        App(Call(Total), 
                                            (ops
                                             |> List.map Value
                                             |> opList))) (genFOfPrim (Gen.listOf))
                                    Gen.map (fun ops -> 
                                        App(Call(Count), 
                                            (ops
                                             |> List.map Value
                                    ...
                                    Gen.map (fun (gp, gp2) -> 
                                        App(Call(NotEquals), 
                                            opList [ Value(gp)
                                                     Value(gp2) ])) twoParamArrayG
                                    Gen.map3 (fun name s ints -> 
                                        App(Call(Repeat), 
                                            opList [ Lam(name, s)
                                                     Value(ints) ])) Arb.generate<_> subtree genInt
...
type GamePrimitiveGen() =
    static member GamePrimitive() : Arbitrary<GamePrimitive> = Arb.fromGen genGp

type DistTypeGen() =
    static member DistType() : Arbitrary<DistType> = genDistType |> Arb.fromGen

type ListTypeGen() =
    static member ListType() : Arbitrary<ListType> = genListType |> Arb.fromGen

type ScalarTypeGen() =
    static member ScalarType() : Arbitrary<ScalarType> = genScalarType |> Arb.fromGen

type TwoSimilarTypeGen() =
    static member TwoSimilarType() : Arbitrary<TwoSimilarTypes> = genTwoSimilarTypes |> Arb.fromGen

type TwoSimilarScalarTypeGen() =
    static member TwoSimilarScalarType() : Arbitrary<TwoSimilarScalarTypes> = genTwoSimilarScalarTypes |> Arb.fromGen

type ListDistScalarTypeGen() =
    static member ListDistScalarType() : Arbitrary<ListDistScalarType> = genListDistScalarType |> Arb.fromGen

type DieGen() =
    static member Die() : Arbitrary<int> = Arb.fromGen genDie

type GenOp() =
    static member Operation() : Arbitrary<Operation> = Arb.fromGen genOp

let config =
    { FsCheckConfig.defaultConfig with arbitrary =
                                           (typeof<GenOp>) 
                                           :: (typeof<DistTypeGen>) 
                                              :: (typeof<ListTypeGen>) 
                                                       ...
                                                       :: (typeof<TwoSimilarScalarTypeGen>) 
                                                          :: (typeof<GamePrimitiveGen>) 
                                                             :: (typeof<ListDistScalarTypeGen>) 
                                                                :: (typeof<DieGen>) :: FsCheckConfig.defaultConfig.arbitrary }

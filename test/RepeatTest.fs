...
let tests = 
    ...
    let ``3 + NoValue = T + Unknown = T`` (value1:GamePrimitive) = 
        ...
        value1Type ==? result
        value2Type ==? Unknown
    let ``3 + 3 = Scalara + Scalara = Scalara`` (TwoScalarType (value1,value2)) = 
        let value1Type = value1 |> Value |> toTyped
        let value2Type = value2 |> Value |> toTyped
        let result = [Value value1;Value value2] |> opList |> call Total >>= "result" |> es "result" |> toTyped
    ...
    let ``3D6 = Repeat D6 3 = Scalara Dist x Scalarb  = Scalara Dist List`` (DistType value1) (ScalarType value2) = 
        ...
        | a,b,c -> failwith <| sprintf "%A, %A, %A don't all have the same dimension" a b c
    let ``Double D6 = Scalara x Scalara Dist = Scalara Dist`` (ScalarType value1) (DistType value2) = 
        let value1Type = value1 |> Value |> toTyped
        let value2Type = value2 |> Value |> toTyped
        let result = [Value value1;Value value2] |> opList |> call Product >>= "result" |> es "result" |> toTyped
        match value1Type,value2Type,result with 
    ...
    testList "Repeat Tests" [
        ...
        testPropertyWithConfig config "NoValue + 3 = Unknown + T = T" ``NoValue + 3 = Unknown + T = T`` 
        testPropertyWithConfig config "3 + NoValue = T + Unknown = T" ``3 + NoValue = T + Unknown = T`` 
        testPropertyWithConfig config "`3 + 3 = Scalara + Scalara = Scalara" ``3 + 3 = Scalara + Scalara = Scalara``
        testPropertyWithConfig config "3 × 3 = Scalara × Scalara = Scalara " ``3 × 3 = Scalara × Scalara = Scalara``
